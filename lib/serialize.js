  ...
  m.reply = async (text, options = {}) => {
            ...
            m.cht, {
                ...
                contextInfo: {
                    ...
                    externalAdReply: {
                        ...
                        mediaType: 1,
                        thumbnail: fs.readFileSync('./image/Hanako-replydoc.jpg'),
   ...
   m.replykontak = async (text, options = {}) => {
      ...
      let getppkontak = await sock.getBuffer(profile)
		...
		if (typeof text === 'string') {
     await sock.sendMessage(m.cht, { document: fs.readFileSync("./image/doc.txt"), fileName: `${Func.Styles(`${config.name}`)}`, mimetype: 'application/msword', jpegThumbnail:await sock.resize(fs.readFileSync("./image/Hanako-replydoc.jpg"), 356, 200), caption: text, mentions: [...sock.parseMention(text)], ...options, contextInfo: { isForwarded: true, forwardingScore: 99999, forwardedNewsletterMessageInfo: { newsletterJid: config.saluran, serverMessageId: -1, newsletterName: `${Func.Styles(`Sc: ${config.name} By: ${config.ownername}`)}` }} }, { quoted: m.fkontak });
		} else if (typeof text === 'object' && typeof text !== 'string') {
	 await sock.sendMessage(m.cht, { ...text, ...options }, { quoted: m.fkontak, ephemeralExpiration: m.expiration, ...options });
		}
    };
  m.react = async (emoji) => {
